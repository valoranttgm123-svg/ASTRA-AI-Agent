...
- **Phase 5A — Bounded Chief orchestrator: IMPLEMENTED ON CURRENT FEATURE BRANCH.** Validated plans can execute step-by-step with dependency, permission, retry, timeout and cancellation boundaries. Memory/reasoning and permitted Codex local engineering paths have real handlers; research/browser and general MCP tools remain truthfully unavailable. Full Phase 5 completion depends on Phase 6/7 real tool coverage.
...
- **Phase 7B — Authenticated GitHub transport + tool-aware planning: IMPLEMENTED ON CURRENT FEATURE BRANCH.** GitHub push/PR/CI are backed by a provider-neutral transport and the default local `gh` CLI adapter becomes READY only after verified `gh auth status`. Strategist plans can carry registered `toolId`/bounded `toolInput`, and the orchestrator executes them through the Tool Runtime. Level-3 UI approval and real target-PC end-to-end GitHub verification remain pending.
- **Next Codex implementation sequence:** follow `docs/SONOR_CODEX_MISSION.md` for the existing local Sonor (preserve/audit/private-backup/reuse existing APIs/minimal compatibility layer/real validation), follow `docs/SONOR_UI_INTEGRATION.md` for the ASTRA-side knowledge UI, then finish Phase 7C with scoped Level-3 approval and real target-PC branch→push→PR→CI verification, add real Research/browser tooling to complete remaining Phase 5 coverage, and later connect production MCP/integrations only when explicitly configured.
