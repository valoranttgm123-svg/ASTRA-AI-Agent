...
- **Phase 5A — Bounded Chief orchestrator: IMPLEMENTED.** Validated plans execute step-by-step with dependency, permission, retry, timeout and cancellation boundaries.
- **Phase 5B — Real Research / Browser: IMPLEMENTED ON CURRENT FEATURE BRANCH.** Public URL fetch is native Level 1 with SSRF/DNS/redirect safety. Source-backed search/research uses a provider-neutral transport with local SearXNG by default and becomes READY only after real health verification. Research evidence carries source IDs/provenance and is treated as untrusted content.
- **Phase 8 — Business Skills: IMPLEMENTED ON CURRENT FEATURE BRANCH.** Finance/Analytics have native deterministic Level-1 tools. Sales/Marketing/Ops/Editor are input-selected local reasoning/drafting skills. CRM/send/publish/database side effects are not implied and remain separate integration work.
- **Phase 6A — Executable Tool Runtime: IMPLEMENTED.** Tool Registry enforces availability, permission, policy, timeout, cancellation, bounded I/O and verified completion before invoking real handlers. MCP uses the same runtime through an explicit injected transport contract; no production MCP server is claimed connected yet.
- **Phase 7A — Scoped Files + local Git: IMPLEMENTED.** Native tools support exact safe project file read/write with SHA preconditions/read-back verification, local Git status/diff/branch/stage/commit, and allowlisted npm verification.
...
- **Next Codex implementation sequence:** follow `docs/SONOR_CODEX_MISSION.md` for the existing local Sonor (preserve/audit/private-backup/reuse existing APIs/minimal compatibility layer/real validation), follow `docs/SONOR_UI_INTEGRATION.md` for the ASTRA-side knowledge UI, then perform target-PC Phase 7 GitHub validation, add real Research/browser tooling to complete remaining Phase 5 coverage, and later connect production MCP/integrations only when explicitly configured.
...
## Phase 14E2 — explicit opt-in local automation service
