...
- **Phase 6A — Executable Tool Runtime: IMPLEMENTED ON CURRENT FEATURE BRANCH.** Tool Registry now enforces availability, permission, policy, timeout, cancellation, bounded I/O and verified completion before invoking real handlers. `project.context.search` is the first native READY Level-1 tool. MCP uses the same runtime through an explicit injected transport contract; no production MCP server is claimed connected yet.
- **Next Codex implementation sequence:** follow `docs/SONOR_CODEX_MISSION.md` for the existing local Sonor (preserve/audit/private-backup/reuse existing APIs/minimal compatibility layer/real validation), follow `docs/SONOR_UI_INTEGRATION.md` for the ASTRA-side knowledge UI, then proceed to Phase 7 Files + GitHub production flow through the executable Tool Runtime, add real Research/browser tooling to complete remaining Phase 5 coverage, and later connect production MCP/integrations only when explicitly configured.
