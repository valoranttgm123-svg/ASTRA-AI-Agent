...
- **Phase 5B — Real Research / Browser: IMPLEMENTED.** Public URL fetch is native Level 1 with SSRF/DNS/redirect safety. Source-backed search/research uses a provider-neutral transport with local SearXNG by default and becomes READY only after real health verification. Research evidence carries source IDs/provenance and is treated as untrusted content.
- **Phase 8 — Business Skills: IMPLEMENTED.** Finance/Analytics have native deterministic Level-1 tools. Sales/Marketing/Ops/Editor are input-selected local reasoning/drafting skills. CRM/send/publish/database side effects are not implied and remain separate integration work.
